...
//필드의 값을 읽고 쓰기 위해서 게터와 세터를 만들어주는 것이 일반적인 문법(스트림부트에서 롬북 사용으로 대체 가능)

public class FieldEx {
    public static void main(String[] args) {
        int localVal = 30; //지역변수, stack 영역에 생성됨, {}범위 내에서만 생존함, 할당 시 초기화되지 않음
        FieldClass field1 = new FieldClass(); //인스턴스(객체) 생성
        FieldClass field2 = new FieldClass(); //두번째 인스턴스 생성

        //인스턴스 필드에서 값 대입 (참조 변수로 접근), 객체의 생성 개수만큼 생성됨
...
class FieldClass {
    int instanceVar = 20; // 인스턴스 변수 or 인스턴스 필드 : 객체생성시 생성됨, 힙 영역에 할당됨
    static int classVar = 10; //클래스 변수 or 클래스 필드 : 클래스 생성시 생성됨. 메소드(static)영역에 할당됨
